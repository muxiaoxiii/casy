...
**大口袋数据管线全线贯通**：
- 知识库自动入库：法条/笔记/邮件自动写入 knowledge_items
- 节假日自动解析：国务院通知 → AI 解析日期 → 提示确认
- 任务自动创建：传票/口审通知 → 自动生成 5 个庭审准备任务
- 期限自动触发：判决书 → 更新 verdict_date → 重算上诉期
- 案件自动更新：起诉状 → 更新 complaint_received_date + court
- 审查意见 → 更新 relief_deadline
